...
#                          tracker_output_format='mot_challenge')
    ...
    def prediction(self):        
        ...
        while getattr(t, "do_run", True):            
            ...
            Fences = list(self.vertex.keys())
            for F in Fences :
                self.detect_target = detect_filter(self.detections, self.target_classes, self.vertex[F])

            # if len(self.detect_target) != 0:
                # self.print_msg(f"偵測到目標 : {self.detect_target}")
            ...
            if self.obj_trace: 
                ...
                for F in Fences :
                    image = draw_boxes(self.detections, image, class_colors, self.target_classes, self.vertex[F])
            
            
            if self.draw_polygon: 
                for F in Fences :
                    image = draw_polylines(image, self.vertex[F])  # draw the polygon
